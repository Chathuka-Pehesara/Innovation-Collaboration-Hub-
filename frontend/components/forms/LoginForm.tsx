...
export function LoginForm() {
  ...
  return (
    ...
    >
      ...
      {/* Google Button */}
      <motion.div variants={itemVariants} className="relative group overflow-hidden rounded-xl">
        {/* Theme-colored hover glowing border background */}
        <div className="absolute inset-0 bg-gradient-to-r from-amber-500/25 via-[#702224]/10 to-orange-500/25 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl blur-sm -z-10" />
        ...
        <motion.a
          ...
          whileTap={{ scale: 0.98 }}
          className="relative flex items-center justify-center gap-3 w-full py-3 px-4 bg-white/70 dark:bg-white/5 backdrop-blur-md border border-amber-900/10 dark:border-white/10 text-amber-950 dark:text-white font-bold rounded-xl text-sm transition-all duration-300 cursor-pointer overflow-hidden"
        >
          {/* Shimmer Effect */}
          <motion.div
            className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-amber-500/15 dark:via-white/10 to-transparent"
            animate={{
              x: ["0%", "200%"],
            }}
            transition={{
